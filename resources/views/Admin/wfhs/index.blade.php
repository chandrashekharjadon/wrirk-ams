...
<div class="container-fluid px-4 py-5">
    ...
    <div class="text-start mb-4">
        <h2 class="fw-bold page-title">
            <i class="bi bi-house-door-fill me-2"></i> Work From Home (WFH) Records
        </h2>
        <p class="text-light small opacity-75">
            Manage and track all employee work-from-home requests and records.
        </p>
    </div>

    <!-- 🔍 Filter Card -->
    <div class="card dashboard-card p-4 mb-4">
        <form method="GET" action="{{ route('admin.wfhs.index') }}" class="row g-3 align-items-end">
            ...
            <div class="col-md-3 d-flex align-items-end">
                ...
                <a href="{{ route('admin.wfhs.index') }}" class="btn btn-secondary reset-btn">
    ...
    <div class="card dashboard-card p-4">
        ...
        <div class="table-responsive">
            <table class="custom-table align-middle mb-0">
                ...
                <tbody>
                    @forelse($wfhRecords as $wfh)
                        <tr>
                            <td>{{ ($wfhRecords->currentPage() - 1) * $wfhRecords->perPage() + $loop->iteration }}</td>
                            <td class="fw-semibold text-light">
                                <i class="bi bi-person-circle me-2 text-info"></i>{{ $wfh->user->name ?? '—' }}
                            </td>
                            <td>{{ \Carbon\Carbon::parse($wfh->date)->format('d M Y') }}</td>
                            <td class="text-center">
                                <span class="badge bg-info text-dark">{{ $wfh->percent }}%</span>
                            </td>
                            <td>{{ $wfh->check_in ?? '—' }}</td>
                            <td>{{ $wfh->check_out ?? '—' }}</td>
                            <td class="small">{{ $wfh->remark ?? '—' }}</td>
                            <td class="text-center">
                                <a href="{{ route('admin.wfhs.edit', $wfh->id) }}" 
                                   class="btn btn-sm btn-outline-warning me-2 rounded-pill px-3">
                                    <i class="bi bi-pencil-fill"></i>
                                </a>

                                <form action="{{ route('admin.wfhs.destroy', $wfh->id) }}" 
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="text-center py-4 text-light opacity-75">
                                <i class="bi bi-info-circle me-2"></i>No WFH records found.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        <div class="d-flex justify-content-end mt-4">
            {{ $wfhRecords->appends(request()->query())->links('pagination::custom') }}
        </div>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    document.querySelectorAll('.delete-btn').forEach(button => {
        button.addEventListener('click', function(e) {
            ...
            Swal.fire({
                title: 'Are you sure?',
                text: "This WFH record will be permanently deleted!",
                icon: 'warning',
